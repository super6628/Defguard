...
///
/// Every requested user/group ID is validated before mutation, preventing a typo or stale UI
/// selection from producing a silently partial operation. Group operations are idempotent.
/// The caller remains responsible for authorization, audit events, gateway updates and LDAP sync.
pub async fn apply(
    pool: &PgPool,
    user_ids: &[Id],
