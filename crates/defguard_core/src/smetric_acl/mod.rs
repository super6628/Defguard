...
fn validate_source(rule: &Rule) -> Result<(), ValidationError> {
    match &rule.source {
        Subject::Any => Ok(()),
        Subject::Cidr(value) => IpNetwork::from_str(value)
            .map(|_| ())
        ...
        | Subject::Location(value) => validate_nonempty(rule.id, value),
    }
}
...
fn validate_destination(rule: &Rule) -> Result<(), ValidationError> {
    match &rule.destination {
        Destination::Any => Ok(()),
        Destination::Cidr(value) => IpNetwork::from_str(value)
            .map(|_| ())
            .map_err(|_| ValidationError::InvalidCidr(rule.id)),
        Destination::Ip(value) => IpAddr::from_str(value)
            .map(|_| ())
        ...
        Destination::IpRange(value) => {
            ...
            let start = IpAddr::from_str(start.trim())
                .map_err(|_| ValidationError::InvalidIpRange(rule.id))?;
        ...
        Destination::Alias(value) | Destination::Service(value) => validate_nonempty(rule.id, value),
    }
}

fn validate_nonempty(rule_id: i64, value: &str) -> Result<(), ValidationError> {
    if value.trim().is_empty() {
        Err(ValidationError::EmptySelector(rule_id))
    } else {
        Ok(())
    }
}
...
mod tests {
    ...
    fn compiler_orders_rules_deterministically() {
        ...
        assert_eq!(compiled.rules[0].priority, 100);
        assert_eq!(compiled.rules[1].priority, 200);
    ...
    fn duplicate_priorities_are_rejected() {
        ...
        assert_eq!(compile(policy), Err(ValidationError::DuplicatePriority(100)));
    }

    #[test]
    fn ports_require_tcp_or_udp() {
        let mut invalid = rule(1, 100);
        invalid.protocol = Protocol::Icmp;
        let policy = Policy {
            id: 1,
            name: "Corporate".into(),
            revision: 1,
        ...
        assert_eq!(compile(policy), Err(ValidationError::PortsWithUnsupportedProtocol(1)));
    }
}
