...
#[utoipa::path(post,path="/api/v1/webhook",tag="webhook",request_body=WebHookData,responses((status=201),(status=400),(status=401,body=ApiErrorResponse),(status=403,body=ApiErrorResponse),(status=500,body=ApiErrorResponse)),security(("cookie"=[]),("api_token"=[])))]
...
#[utoipa::path(get,path="/api/v1/webhook",tag="webhook",responses((status=200,body=[WebHook]),(status=401,body=ApiErrorResponse),(status=403,body=ApiErrorResponse),(status=500,body=ApiErrorResponse)),security(("cookie"=[]),("api_token"=[])))]
...
#[utoipa::path(get,path="/api/v1/webhook/{id}",tag="webhook",params(("id"=i64,Path)),responses((status=200,body=WebHook),(status=401,body=ApiErrorResponse),(status=403,body=ApiErrorResponse),(status=404),(status=500,body=ApiErrorResponse)),security(("cookie"=[]),("api_token"=[])))]
...
#[utoipa::path(put,path="/api/v1/webhook/{id}",tag="webhook",request_body=WebHookData,params(("id"=i64,Path)),responses((status=200),(status=400,body=ApiErrorResponse),(status=401,body=ApiErrorResponse),(status=403,body=ApiErrorResponse),(status=404),(status=500,body=ApiErrorResponse)),security(("cookie"=[]),("api_token"=[])))]
...
#[utoipa::path(delete,path="/api/v1/webhook/{id}",tag="webhook",params(("id"=i64,Path)),responses((status=200),(status=401,body=ApiErrorResponse),(status=403,body=ApiErrorResponse),(status=404),(status=500,body=ApiErrorResponse)),security(("cookie"=[]),("api_token"=[])))]
pub async fn delete_webhook(_admin: AdminRole, State(appstate): State<AppState>, session: SessionInfo, context: ApiRequestContext, Path(id): Path<Id>) -> ApiResult {
    let status = match WebHook::find_by_id(&appstate.pool, id).await? {
        Some(webhook) => {
            ...
            appstate.emit_event(ApiEvent { context, event: Box::new(ApiEventType::WebHookRemoved { webhook: redact_webhook_token(webhook) }) })?;
            StatusCode::OK
        }
        None => StatusCode::NOT_FOUND,
    };
...
#[utoipa::path(post,path="/api/v1/webhook/{id}",tag="webhook",request_body=ChangeStateData,params(("id"=i64,Path)),responses((status=200),(status=400,body=ApiErrorResponse),(status=401,body=ApiErrorResponse),(status=403,body=ApiErrorResponse),(status=404),(status=502,body=WebHookTestResponse),(status=500,body=ApiErrorResponse)),security(("cookie"=[]),("api_token"=[])))]
pub async fn change_enabled(_admin: AdminRole, session: SessionInfo, context: ApiRequestContext, State(appstate): State<AppState>, Path(id): Path<Id>, Json(data): Json<ChangeStateData>) -> ApiResult {
    if data.test && data.enabled.is_some() {
        return Err(WebError::BadRequest("Webhook action must specify exactly one of enabled or test=true".into()));
    }
    if data.test {
    ...
    let status = match WebHook::find_by_id(&appstate.pool, id).await? {
        Some(mut webhook) => {
            ...
            appstate.emit_event(ApiEvent { context, event: Box::new(ApiEventType::WebHookStateChanged { enabled, webhook: redact_webhook_token(webhook) }) })?;
            StatusCode::OK
        }
        None => StatusCode::NOT_FOUND,
    };
...
mod tests {
    ...
    fn preserves_legacy_enabled_action_payload() {
        let data: ChangeStateData = serde_json::from_value(serde_json::json!({"enabled": true})).unwrap();
    ...
    fn accepts_test_action_payload() {
        let data: ChangeStateData = serde_json::from_value(serde_json::json!({"test": true})).unwrap();
